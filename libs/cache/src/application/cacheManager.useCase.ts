...
export class CacheManagerUseCase implements CacheAdapter {
  ...
  async saveCacheWithExpiration(key: string, value: any, minutes: number): Promise<void> {
    return this.saveCacheRepository.saveCacheWithExpiration(key, value, minutes);
  }
  ...
  async updateCacheWithExpiration(key: string, value: any, minutes: number): Promise<void> {
    return this.updateCacheRepository.updateCacheWithExpiration(key, value, minutes);
  }
}
