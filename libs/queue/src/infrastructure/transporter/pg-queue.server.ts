...
export class PgQueueServer extends Server implements CustomTransportStrategy {
  ...
  private async processJob(
    ...
    try {
      ...
      this.logger.error(`Job ${job.id} failed (attempt ${job.attempts}/${job.maxRetries}): ${message}`);
  ...
  private async markFailed(id: string, message: string, reschedule: boolean): Promise<void> {
    if (reschedule) {
      await this.pool.query(
  ...
  private async drainInflight(timeoutMs: number): Promise<void> {
    ...
    if (this.inflightCount > 0) {
      this.logger.warn(`Shutdown with ${this.inflightCount} jobs still in flight`);
    }
  }
  ...
  private async reconnect(): Promise<void> {
    ...
    while (!this.closing) {
      try {
        ...
        client.on('notification', (msg) => void this.handleNotification(msg.channel));
        client.on('error', (err) => {
          this.logger.error(`pg LISTEN client error: ${err.message}`);
          void this.reconnect();
        });
        ...
        const message = err instanceof Error ? err.message : String(err);
        this.logger.error(`Reconnect failed, retrying in ${delay}ms: ${message}`);
