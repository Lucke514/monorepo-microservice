...
import { EnqueueJobRepository, EnqueueOptions } from '../../domain/ports/enqueue-job.repository.js';
...
export class EnqueueJobService implements EnqueueJobRepository {
  ...
  async enqueue(
    ...
    await this.dataSource.query('SELECT pg_notify($1, $2)', [queueName, saved.id]);
    return saved.id;
  }
}
